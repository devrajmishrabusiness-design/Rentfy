...
export type EngineStatus = 'idle' | 'initializing' | 'running' | 'paused' | 'stopping' | 'stopped' | 'error';
...
export interface PluginDefinition<Input = unknown, Output = unknown, Config = unknown> {
    ...
    priority?: number;
    phase?: string;
    execute: (input: Input, context: PluginExecutionContext<Config>) => Promise<Output>;
    validateConfig?: (config: Config) => boolean;
    onInit?: (context: PluginInitContext<Config>) => Promise<void>;
    onShutdown?: (context: PluginShutdownContext) => Promise<void>;
}
...
export type { EngineErrorCode, PluginError, LifecycleError } from './errors';
